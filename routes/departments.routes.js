...
router.get('/departments/random', (req, res) => {
  //aggregate - losowy element z kolekcji (size = ile elementow chcemy), zawsze zwraca zbior danych, nawet jak size: 1
  req.db
    .collection('departments')
...
router.put('/departments/:id', (req, res) => {
  ...
  req.db.collection('departments').updateOne({_id: ObjectId(req.params.id)}, {$set: {name: name}}, err => {
    if (err) res.status(500).json({message: err + ' ' + req.params.id});
    else res.json({message: 'OK'});
  });
});
